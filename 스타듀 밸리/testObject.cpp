...
void testObject::render()
{
	...
	case OBJECT::TREE1: case OBJECT::TREE2: case OBJECT::TREE3:
		_image->frameRender(getMemDC(), -CAMX + _pos.x, -CAMY + _pos.y, _frameX, _frameY);
		_image->frameRender(getMemDC(), -CAMX + _pos.x, -CAMY + _pos.y - TILESIZE, _frameX, _frameY - 1);
		for (int j = -5; j <= -2; ++j)
		{
			for (int i = -1; i <= 1; ++i)
			{
				_image->frameRender(getMemDC(), -CAMX + _pos.x + i * TILESIZE, -CAMY + _pos.y + j * TILESIZE, _frameX + i, _frameY + j);
			}
		}
		break;
